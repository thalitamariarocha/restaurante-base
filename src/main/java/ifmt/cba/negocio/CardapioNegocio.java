...
public class CardapioNegocio {
	...
	public void inserir(CardapioDTO cardapioDTO) throws NegocioException {
		...
		try {
			if (!cardapioDAO.buscarPorNome(cardapio.getNome()).isEmpty()) {
	...
	public List<CardapioDTO> pesquisaPorNome(String nome) throws NegocioException {
		try {
			return this.toDTOAll(cardapioDAO.buscarPorNome(nome));
		} catch (PersistenciaException ex) {
